...
public class Main {
    ...
    public static void main(String[] args) {
        var number = "Н555НН100";//зачем мы создаем этот номер?
        ...
        String time = Long.toString(time2 - time1);//вычитаем для чего?
        System.out.println("Поиск перебором: номер " + seach + ", поиск занял " + time + "нс");

        List sortedList = new ArrayList(list);
        Collections.sort(sortedList);//мы не видим то, что сортируем поэтому зачем сортируем?
